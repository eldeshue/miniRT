...
/*   Updated: 2024/08/26 16:12:49 by dogwak           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_render_resource(t_render_resource *self, char **argv)
{
	...
	// camera init
	self->fov = 120;
	self->cam.vpos = ftmf4_set_vector(0.0f, -50.0f, 80.0f, 1.0f);
	self->cam.vtarget = ftmf4_set_vector(0.0f, 1.0f, -0.5f, 0.0f);
	...
	// cylinder
	tmp.m.obj_color = ftmf4_set_vector(55.0f, 155.0f, 55.0f, 0.0f);	// green
	tmp.p1 = ftmf4_set_vector(0.0f, 0.0f, 60.0f, 1.0f);
	tmp.p2 = ftmf4_set_vector(0.0f, 0.0f, 10.0f, 0.0f);	// normal vector of cylinder
	tmp.val = 20.0f;
	tmp.type = cylinder;
	//self->render_objects->push_back(self->render_objects, &tmp);
	...
	tmp.p2 = ftmf4_set_vector(0.0f, 0.0f, 70.0f, 1.0f);  // vertex
	tmp.p1 = ftmf4_set_vector(0.0f, 0.0f, 30.0f, 1.0f);  // center
	tmp.val = 40.0f;
	tmp.type = cone;
	//self->render_objects->push_back(self->render_objects, &tmp);
